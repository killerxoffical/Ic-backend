// --- START: server.js (v23 - Fixed Future Market Freeze & Admin Loading) ---
...
const FUTURE_DELAY_MS = 5 * 60 * 1000; 
...
setInterval(() => {
  ...
  for (const marketId in markets) {
    ...
    if (m.type === 'future') {
        ...
        let isFutureQueueUpdated = false;

        // Generate future candles up to 5 minutes ahead
        while (lastFuture && lastFuture.timestamp < currentPeriod + FUTURE_DELAY_MS) {
            ...
            isFutureQueueUpdated = true;
        }

        // Clean up old future candles (Keep the current period!)
        const initialLen = m.futureQueue.length;
        m.futureQueue = m.futureQueue.filter(c => c.timestamp >= currentPeriod); // <-- Fixed Bug Here
        if (m.futureQueue.length !== initialLen) isFutureQueueUpdated = true;

        // Push to Firebase instantly so admin panel doesn't freeze
        if (isFutureQueueUpdated) {
            db.ref(`admin/markets/${marketId}/future_data`).set(m.futureQueue).catch(()=>{});
        }
        ...
        if (targetCandle) {
            ...
            // Smooth Ticking for Future Market
            const progress = (now - currentPeriod) / TIMEFRAME;
            let currentLivePrice = interpolatePrice(targetCandle, progress);
            const noise = (Math.random() - 0.5) * (targetCandle.open * 0.00008); 
            currentLivePrice += noise;
