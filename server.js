// --- START: main app server.js (v42.0 - Quotex Stutter Jumps & 15m Pumps) ---
...
const TICK_MS = 100; // Faster server loop to catch immediate jumps
...
// 🔥 THE NEW PURE RANDOM MARKET ENGINE & 15m PUMPS 🔥
function generateRealisticCandle(marketData, timestamp, open, forcedDirection = null, isLive = false) {
    ...
    // Track candles for the 10-15 minute Big Pump/Dump
    marketData.candleCount = (marketData.candleCount || 0) + 1;
    let isPumpCandle = false;

    if (!marketData.nextPumpTarget) {
        marketData.nextPumpTarget = 10 + Math.floor(Math.random() * 6); // Randomly between 10 to 15
    }

    if (marketData.candleCount >= marketData.nextPumpTarget) {
        isPumpCandle = true;
        marketData.candleCount = 0;
        marketData.nextPumpTarget = 10 + Math.floor(Math.random() * 6);
    }

    // Evolve Trend & Volatility
    marketData.trendBias *= 0.85; 
    marketData.trendBias += (Math.random() - 0.5) * 0.30; 
    marketData.trendBias = Math.max(-0.6, Math.min(0.6, marketData.trendBias));

    marketData.volatility += (Math.random() - 0.5) * 0.20;
    marketData.volatility = Math.max(0.6, Math.min(2.0, marketData.volatility));
    ...
    // 🔥 If it's the 10-15 min pump, make it HUGE
    if (isPumpCandle) {
        body = baseVol * (4.0 + Math.random() * 3.0); // 4x to 7x normal size
        upWick = baseVol * (Math.random() * 0.2); // Very tiny wicks
        dnWick = baseVol * (Math.random() * 0.2);
        marketData.trendBias = isGreen ? 0.6 : -0.6; // Shift momentum
    } 
    else {
        ...
        if (typeRoll < 0.15) {
            ...
            body = baseVol * (0.5 + Math.random() * 1.2);
            upWick = baseVol * (0.1 + Math.random() * 1.0);
            dnWick = baseVol * (0.1 + Math.random() * 1.0);
        }
    }
...
function generateAdminCandle(timestamp, open, command, cloneData) {
    ...
    else if (command.includes('MARUBOZU')) { body = baseVol * 3.5; upWick = 0; dnWick = 0; } 
...
async function initializeNewMarket(marketId) {
    ...
    markets[marketId] = { marketId, marketPath: path, trendBias: 0, volatility: 1.0, candleCount: 0, nextPumpTarget: 12, history: [], currentPrice: startPrice };
...
function updateRealisticPrice(marketData, candle, currentPeriod) {
    ...
    // 1. Pre-calculate fixed Waypoints (Discrete steps instead of fluid sliding)
    if (!candle.waypoints) {
        candle.waypoints = [];
        const totalJumps = 30 + Math.floor(Math.random() * 20); // 30 to 50 sudden jumps per minute

        for(let i=0; i<totalJumps; i++) {
            if (i === 0) {
                candle.waypoints.push({ pct: 0, price: candle.open });
            } else if (i === totalJumps - 1) {
                candle.waypoints.push({ pct: 1.0, price: candle.targetClose });
            } else {
                const timePct = (i / totalJumps) + (Math.random() * 0.02 - 0.01);
                
                // Base direction
                let expectedBase = candle.open + (candle.targetClose - candle.open) * timePct;
                
                // Add heavy, chaotic noise to make it zigzag
                let noise = (Math.random() - 0.5) * Math.abs(candle.open * 0.00015);
                
                candle.waypoints.push({ pct: timePct, price: expectedBase + noise });
            }
        }
        
        candle.waypoints.sort((a, b) => a.pct - b.pct);
        
        // Ensure Wicks are hit exactly
        candle.waypoints[Math.floor(totalJumps * 0.3)].price = candle.targetHigh;
        candle.waypoints[Math.floor(totalJumps * 0.7)].price = candle.targetLow;

        candle.nextJumpTime = now + 100;
        marketData.currentPrice = candle.open;
    }

    // 2. STUTTER JUMP LOGIC (Freeze and Snap)
    // Only move if the freeze timer has expired
    if (now >= candle.nextJumpTime) {
        
        // Find where the price SHOULD be right now
        let targetWp = candle.waypoints.find(wp => wp.pct >= progress);
        if (!targetWp) targetWp = candle.waypoints[candle.waypoints.length - 1];

        // INSTANTLY SNAP to the new price (No smoothing)
        marketData.currentPrice = targetWp.price;

        // Calculate the next freeze duration (Quotex effect)
        let delay;
        const delayRoll = Math.random();
        if (delayRoll < 0.15) delay = 50 + Math.random() * 100; // Ultra fast rapid fire ticks
        else if (delayRoll < 0.70) delay = 400 + Math.random() * 800; // Normal sticky pause
        else delay = 1000 + Math.random() * 1500; // Long manipulative freeze

        candle.nextJumpTime = now + delay;
    }

    // Lock perfectly to close at the end
...
setInterval(async () => {
    try {
        await db.ref('admin/server_status').set({ lastActive: Date.now(), version: 'V42.0' });
...
app.get('/ping', async (_req, res) => {
    res.send('Server V42.0 - Ultra Realistic Stutter Jumps & 15m Pumps Active');
